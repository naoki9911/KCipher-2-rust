...
fn sub_k2(t: u32) -> u32 {
    ...
    let w0 = (t & 0xFF) as u8;
    let w1 = ((t >> 8) & 0xFF) as u8;
    let w2 = ((t >> 16) & 0xFF) as u8;
    let w3 = ((t >> 24) & 0xFF) as u8;

    let t0 = S_box[w0 as usize];
    let t1 = S_box[w1 as usize];
    let t2 = S_box[w2 as usize];
    let t3 = S_box[w3 as usize];
    ...
    out = ((q3 as u32) << 24) | ((q2 as u32) << 16) | ((q1 as u32) << 8) | (q0 as u32);
    return out;
}
...
*/
fn key_expansion(ctx:&mut Context, key:&[u32; 4], iv:&[u32; 4]) {
    ctx.IV[0] = iv[0];
    ctx.IV[1] = iv[1];
    ctx.IV[2] = iv[2];
    ctx.IV[3] = iv[3];
...
*/
fn setup_state_values(ctx:&mut Context, key:&[u32; 4], iv:&[u32; 4]) {
...
* @modify   S       : (OUTPUT), (A, B, L1, R1, L2, R2)
*/
fn init(ctx:&mut Context, key:&[u32; 4], iv:&[u32; 4]) {
    setup_state_values(ctx, key, iv);
    for _ in 0..24 {
        next(ctx, INIT);
    }
}
...
* @param    A   : (INPUT), 32 bits
* @param    B   : (INPUT), 32 bits
* @param    C   : (INPUT), 32 bits
* @param    D   : (INPUT), 32 bits
* @return       : (OUTPUT), 32 bits
*/
fn NLF(A: u32, B: u32, C: u32, D:u32) -> u32 {
    let Q = (A + B) ^ C ^ D;
    return Q;
}
...
* @modify   S       : (OUTPUT)
*/
fn next (ctx:&mut Context, mode:u32) {
    let mut nA:[u32; 5] = [0;5];
    let mut nB:[u32; 11] = [0; 11];
    let mut temp1:u32 = 0;
    let mut temp2:u32 = 0;

    let nL1 = sub_k2(ctx.R2 + ctx.B[4]);
    let nR1 = sub_k2(ctx.L2 + ctx.B[9]);
    let nL2 = sub_k2(ctx.L1);
    let nR2 = sub_k2(ctx.R1);

    // m = 0 ... 3
    nA[0] = ctx.A[1];   nA[1] = ctx.A[2];   nA[2] = ctx.A[3];   nA[3] = ctx.A[4];

    // m = 0 ... 9
    nB[0] = ctx.B[1];   nB[1] = ctx.B[2];   nB[2] = ctx.B[3];   nB[3] = ctx.B[4];
    nB[4] = ctx.B[5];   nB[5] = ctx.B[6];   nB[6] = ctx.B[7];   nB[7] = ctx.B[8];
    nB[8] = ctx.B[9];   nB[9] = ctx.B[10];

    // update nA[4]
    temp1 = (ctx.A[0] << 8) ^ amul0[(ctx.A[0] >> 24) as usize];
    nA[4] = temp1 ^ ctx.A[3];
    if mode == INIT {
    ...
    // update nB[10]
    if (ctx.A[2] & 0x40000000 != 0) /* if A[2][30] == 1 */ {
        temp1 = (ctx.B[0] << 8) ^ amul1[(ctx.B[0] >> 24) as usize];
    } else /*if A[2][30] == 0*/ {
        temp1 = (ctx.B[0] << 8) ^ amul2[(ctx.B[0] >> 24) as usize];
    }

    if (ctx.A[2] & 0x80000000 != 0) /* if A[2][31] == 1 */ {
        temp2 = (ctx.B[8] << 8) ^ amul3[(ctx.B[8] >> 24) as usize];
    } else /* if A[2][31] == 0 */ {
        temp2 = ctx.B[8];
    }
    ...
    /* copy S' to S */
    ctx.A[0] = nA[0];   ctx.A[1] = nA[1];   ctx.A[2] = nA[2];
    ctx.A[3] = nA[3];   ctx.A[4] = nA[4];

    ctx.B[0] = nB[0];   ctx.B[1] = nB[1];   ctx.B[2] = nB[2];   ctx.B[3] = nB[3];
    ctx.B[4] = nB[4];   ctx.B[5] = nB[5];   ctx.B[6] = nB[6];   ctx.B[7] = nB[7];
    ctx.B[8] = nB[8];   ctx.B[9] = nB[9];   ctx.B[10] = nB[10];

    ctx.L1 = nL1;   ctx.R1 = nR1;   ctx.L2 = nL2;   ctx.R2 = nR2;
}
...
*/
fn stream(ctx: &Context) -> (u32, u32) {
    let zh = NLF(ctx.B[10], ctx.L2, ctx.L1, ctx.A[0]);
    let zl = NLF(ctx.B[0], ctx.R2, ctx.R1, ctx.A[4]);
    return (zh, zl);
}

fn main() {
    println!("Hello, world!");
    let key:[u32; 4] = [0u32; 4];
    let iv:[u32; 4] = [0u32; 4];
    //let key:[u32; 4] = [0x0F1E2D3C, 0x4B5A6978, 0x8796A5B4, 0xC3D2E1F0];
    //let iv:[u32; 4] = [0xF0E0D0C0, 0xB0A09080, 0x70605040, 0x30201000];
    let mut ctx = Context{
        A:[0u32; 5],
        B:[0u32; 11],
        L1:0,
